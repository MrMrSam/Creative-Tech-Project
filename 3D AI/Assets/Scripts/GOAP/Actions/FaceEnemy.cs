...
public class FaceEnemy : GoapAction
{
	...
	public override bool Test(GoapWorldstate _worldState)
	{
		...
		if ((enemyTroct.transform.position - core.actor.currentTrOct.transform.position).normalized == transform.forward)
		{
			...
			for (int j = 0; j < _worldState.allies.Count; j++)
			{
				...
				if (((allyTroct.transform.position - core.actor.currentTrOct.transform.position).normalized == transform.forward) &&
