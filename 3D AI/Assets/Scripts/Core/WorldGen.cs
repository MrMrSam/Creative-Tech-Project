...
public class WorldGen : MonoBehaviour
{
	...
	private List<GameObject> truncOcts;

	public GameObject Cam;

	private GameObject allTrOcts;

	public bool deadSpace, seeThroughTrOcts;

	private List<GameObject> deadTrocts;

	private bool updated;

	private static WorldGen m_instance = null;
	public static WorldGen instance { get { return m_instance; } }
	...
	public List<GameObject> GenWorld(float _distance)
	{
		...
		while (!connectionsFull())
		{
			...
			for (int i = 0; i < truncOcts.Count; i++)
			{
				...
				for (int j = 0; j < 14; j++)
				{
					...
					{
						SpawnTrunc(truncOcts[i], j, _distance);
					}
				}
			}
		}
		...
		allTrOcts.transform.parent = transform;

		//return the list of TrOcts
		return truncOcts;
	}
	...
	void SpawnTrunc (GameObject _trOct, int _connection, float _distance)
	{
		...
		if (Vector3.Distance(newTrOct.transform.position, Vector3.zero) < _distance)
		{
	...
	public int ConvertConnecton(int _connection)
	{
		int connecting = -1;
