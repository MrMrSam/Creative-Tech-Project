...
public class TeamManager : MonoBehaviour 
{
	...
	public void ClearFOW(int _team)
	{
		...
		for (int i = 0; i < enemyTeam.members.Count; i++)
		{
			...
			if (activeActors.Contains(enemyTeam.members[i]))
			{
				//if it is not/no longer inside the FOW
				if (!enemyTeam.members[i].GetComponent<ActorBase>().currentTrOct.GetComponent<TruncOct>().inFow)
				{
					//set to visible/active
					enemyTeam.members[i].GetComponent<MeshRenderer>().enabled = true;
				}
			}
		}
	}
